namespace DocumentDB.Samples.Twitter
{
    ...
    public class OAuthBase
    {
        ...
        {
            return this.GenerateSignature(url, consumerKey, consumerSecret, token, tokenSecret, httpMethod, timeStamp, nonce, SignatureTypes.HMACSHA1, out normalizedUrl, out normalizedRequestParameters, out authHeader);
        }
        ...
        public string GenerateSignature(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, SignatureTypes signatureType, out string normalizedUrl, out string normalizedRequestParameters, out string authHeader)
        {
            ...
            switch (signatureType)
            {
                ...
                case SignatureTypes.HMACSHA1:
                    ...
                    string signature = this.GenerateSignatureUsingHash(signatureBase, hmacsha1);

                    StringBuilder auth = new StringBuilder();
                    auth.AppendFormat("{0}=\"{1}\", ", OAuthConsumerKeyKey, this.UrlEncode(consumerKey));
                    auth.AppendFormat("{0}=\"{1}\", ", OAuthNonceKey, this.UrlEncode(nonce));
                    auth.AppendFormat("{0}=\"{1}\", ", OAuthSignatureKey, this.UrlEncode(signature));
                    auth.AppendFormat("{0}=\"{1}\", ", OAuthSignatureMethodKey, "HMAC-SHA1");
                    auth.AppendFormat("{0}=\"{1}\", ", OAuthTimestampKey, timeStamp);
                    auth.AppendFormat("{0}=\"{1}\", ", OAuthTokenKey, this.UrlEncode(token));
